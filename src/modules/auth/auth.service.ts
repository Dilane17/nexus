...
export class AuthService {
  ...
  async resendOtp(dto: ResendOtpDto): Promise<{ message: string }> {
    if (dto.phone) {
      const user = await this.prisma.user.findUnique({
        where: { phone: dto.phone },
        select: { id: true, firstName: true, phone: true, isPhoneVerified: true },
      });
  ...
  async refresh(
    ...
    if (!tokenMatch) {
      ...
      throw new UnauthorizedException('Token invalide. Session révoquée par sécurité.');
    }
  ...
  private async generateTokens(
    ...
    const refreshExpiry = (this.config.get('JWT_REFRESH_EXPIRES_IN') ?? '30d') as Expiry;
  ...
  private async buildAuthResponse(userId: string, phone: string): Promise<AuthTokens> {
